...
Using the actual PVGIS web interface for years 2005-2023
"""
...
def fetch_pvgis_hourly_data_turin():
    """
    Fetch real PVGIS hourly data for Turin, Italy using the correct parameters
    Based on the PVGIS interface at https://re.jrc.ec.europa.eu/pvg_tools/en/
    """
    ...
    print("Years: 2005-2023 (19 years of data)")
    
    # PVGIS API endpoint for hourly data
    base_url = "https://re.jrc.ec.europa.eu/api/v5_2/timeseries"
    
    # Parameters for Turin, Italy - Request hourly data for multiple years
    params = {
        'lat': 45.0703,           # Latitude for Turin
        'lon': 7.6869,            # Longitude for Turin
        'startyear': 2005,        # Start year
        'endyear': 2023,          # End year (19 years of data)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if hourly_data:
                ...
                daily_average = total_annual / (19 * 365)  # 19 years
                peak_generation = df['pv_generation_kw'].max()
                capacity_factor = total_annual / (120 * 19 * 8760)  # 19 years
                
                print(f"📈 Real PVGIS Statistics (2005-2023):")
                print(f"   Total generation: {total_annual:.0f} kWh")
...
def fetch_pvgis_monthly_data_turin():
    """
    Fetch PVGIS monthly data as fallback for Turin
    """
    print("🌞 Fetching PVGIS monthly data as fallback for Turin...")
    ...
    params = {
        ...
        'userhorizon': '',        # No custom horizon
        'raddatabase': 'PVGIS-SARAH2',  # Use SARAH2 database
        'startyear': 2020,        # Use 2020 as representative year
        'endyear': 2020,          # Single year for monthly data
        'pvcalculation': 1,       # PV calculation
        'pvtechchoice': 'crystSi', # Crystalline silicon
        'mountingplace': 'free',  # Free-standing
        'trackingtype': 0,        # Fixed mounting
        ...
        if response.status_code == 200:
            ...
            if 'outputs' in data and 'monthly' in data['outputs']:
                ...
                if 'E_m' in df.columns:  # Monthly energy in kWh
                    ...
                    return create_hourly_from_monthly_turin(monthly_generation)
                else:
...
def create_hourly_from_monthly_turin(monthly_generation):
    """
    Create realistic hourly profiles from PVGIS monthly data for Turin
    """
    print("📊 Creating hourly profiles from real PVGIS monthly data for Turin...")
    
    # Days per month (2020 is leap year)
    ...
    for month in range(12):
        ...
        for day in range(days_in_month):
            ...
            for hour in range(24):
                # Create realistic hourly pattern based on solar angle for Turin
                if hour < 6 or hour > 18:
                    # Night time - no generation
                    hourly_generation = 0
                else:
                    # Day time - create realistic pattern for Turin latitude
    ...
    print(f"📈 Generated Statistics (from real PVGIS monthly data for Turin):")
...
def save_real_pv_data_turin(df):
    """
    Save real PVGIS data for Turin to CSV files
    """
    if df is None:
    ...
    print(f"✅ Saved real PVGIS data for Turin: {pv_8760_file}")
    ...
    if day_180_end <= len(df):
        ...
        print(f"✅ Saved 24-hour real PVGIS data for Turin: {pv_24h_file}")
        ...
        print(f"📊 24-hour real data for Turin:")
...
def main():
    ...
    try:
        ...
        pv_df = fetch_pvgis_hourly_data_turin()
        
        # If hourly data fails, try monthly data
        if pv_df is None:
            print("\n🔄 Hourly data not available, trying monthly data...")
            pv_df = fetch_pvgis_monthly_data_turin()
        
        if pv_df is not None:
            print()
            
            # Save files
            success = save_real_pv_data_turin(pv_df)
            
            if success:
                print()
                print("=" * 60)
                print("✅ REAL PVGIS DATA FOR TURIN FETCHED SUCCESSFULLY")
                print("=" * 60)
                print("Files created:")
