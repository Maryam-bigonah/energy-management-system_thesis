...
def merge_pv_weather_sources(
    ...
    """
    Merge measured PV power with PVGIS weather into a single hourly DataFrame
    suitable for the PV forecaster.

    NOTE: OpenWeather data is no longer used. The openweather_hourly parameter
    ...
    openweather_hourly : pd.DataFrame, optional
        DEPRECATED: This parameter is ignored. Only PVGIS data is used.
    ...
    # Inner join to ensure we only keep timestamps where PVGIS data exists.
    df = pv_df.join(pvgis_hourly, how="inner")

    # OpenWeather is no longer used - ignore if provided
